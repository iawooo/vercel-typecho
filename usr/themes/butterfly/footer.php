...
document.addEventListener('DOMContentLoaded', function() {
                        ...
                        fetch(path)
                            ...
                            .then(text => {
                                try {
                                    ...
                                    window.searchData = [...xml.querySelectorAll('entry')].map(item => {
                                        return {
    ...
    function initSearchInputListener() {
        ...
        $input.addEventListener('input', function() {
            ...
            if (window.searchData && window.searchData.length > 0) {
                window.searchData.forEach(function(data) {
                    let isMatch = true;
                    ...
                    if (isMatch) {
                        count++;
                        str += '<div class="local-search__hit-item"><a href="' + dataUrl + 
                              '" class="search-result-title">' + dataTitle + '</a>';
                        
                        if (dataContent) {
                            const firstOccur = dataContent.indexOf(keywords[0]);
            ...
            if (count === 0) {
                str += '<div id="local-search__hits-empty">找不到结果: ' + this.value.trim() + '</div>';
            }
    ...
    try {
        ...
        if (typeof Typed !== 'undefined') {
            const typedElement = document.getElementById('subtitle');
            if (!typedElement) {
                console.log('打字机效果的目标元素不存在');
            }
        }
    } catch (e) {
        console.error('错误修复脚本出错:', e);
    }
});
</script>
